...
class VentaCreateSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if len(items) == 0:
            raise serializers.ValidationError({"items": ["Debe enviar al menos un item."]})

        for i, item in enumerate(items):
            cantidad = item.get("cantidad")
            if cantidad is None:
                raise serializers.ValidationError({"items": [f"Item #{i+1}: cantidad es requerida."]})
            if Decimal(str(cantidad)) <= 0:
                raise serializers.ValidationError({"items": [f"Item #{i+1}: cantidad debe ser > 0."]})
    ...
    def create(self, validated_data):
        ...
        for item in items:
            ...
            try:
                ...
                raise serializers.ValidationError({"items": [f"Producto no existe: {producto_id}"]})
            ...
            if cantidad_restante > 0:
                raise serializers.ValidationError(
                    {"items": [f"Stock insuficiente para el producto {producto.nombre}. Faltan: {cantidad_restante}"]}
                )
